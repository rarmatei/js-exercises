...
*/
function travelDestinationsLessThan500Kms(destination) {
  if (destination.distanceKms <= 500) {
    return destination.destinationName;
  }
}
function destinationReachByFerry(destination) {
  if (destination.transportations.includes("ferry")) {
    return destination.destinationName;
  }
}
function DestNameMoreThan300KmsByTrain(destination) {
  if (
...
var destinationNameReachableByFerry = travelDestinations
  ...
  .map(destinationReachByFerry); // Complete here
